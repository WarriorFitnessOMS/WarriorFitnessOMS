...
const SuccessStories = () => {
  ...
  return (
    <section className="w-full py-20 px-4 md:px-6 bg-black">
      ...
      >
        <motion.h2
          className="text-5xl md:text-6xl font-BabesNeue text-white mb-4"
          initial={{ opacity: 0 }}
        ...
        <motion.p
          className="text-lg md:text-2xl font-bold text-orange-500"
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
        ...
        >
          {stories.map((story) => (
            <SwiperSlide key={story.id}>
              ...
              >
                <motion.div
                  className="h-full p-6 rounded-lg border-2 border-orange-500 bg-gray-900/50 backdrop-blur-sm hover:bg-gray-900 transition-all duration-300"
                  whileHover={{
                ...
                >
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-6">
                    {/* BEFORE */}
                    <motion.div
                      className="relative h-48 bg-gray-700 rounded-lg overflow-hidden"
                    ...
                    >
                      ...
                      <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/80 to-transparent p-2">
                        <span className="text-orange-500 font-bold text-sm">BEFORE</span>
                      </div>
                    </motion.div>

                    {/* AFTER */}
                    <motion.div
                      className="relative h-48 bg-gray-600 rounded-lg overflow-hidden"
                      whileHover={{ scale: 1.05 }}
                      transition={{ duration: 0.3 }}
                    >
                      <img 
                      ...
                      />
                      <div className="absolute bottom-0 left-0 right-0 bg-linear-to-t from-black/80 to-transparent p-2">
                        <span className="text-orange-500 font-bold text-sm">AFTER</span>
                      </div>
                    </motion.div>
                  </div>
                  ...
                  {/* QUOTE */}
                  <motion.p
                    className="text-gray-300 text-sm leading-relaxed mb-4 h-20"
                    initial={{ opacity: 0 }}
                    whileInView={{ opacity: 1 }}
